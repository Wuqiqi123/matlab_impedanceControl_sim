clear;
Ftimeseries=load('compare_force_contact_Fe_kenvir.mat');
Xtimeseries=load('compare_force_contact_X_kenvir.mat');
DesiredXtimeseries=load('compare_force_contact_DesiredX_kenvir.mat');
DesiredFetimeseries=load('compare_force_contact_DesiredFe_kenvir.mat');
...
set(Ax(1),'ylim',[-0.5 0.8]) % handles可以指定具体坐标轴的句柄
set(Ax(2),'YLim',[-50 600],'YColor','magenta') % handles可以指定具体坐标轴的句柄
%set(h1,'XTick',0:1:10) % handles可以指定具体坐标轴的句柄
set(Ax(1),'YTick',-0.5:0.1:0.8) % handles可以指定具体坐标轴的句柄
set(Ax(2),'YTick',-50:50:600) % handles可以指定具体坐标轴的句柄
